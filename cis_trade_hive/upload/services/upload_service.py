            ...
            STANDARDIZE_SELECT = {
                ...
                'cis_user_sta_adhoc_position_1': f"""
                    SELECT
                        ...
                        isin_code                               AS isin,
                        NULL                                    AS ticker,
                        CAST(quantity_today AS STRING)          AS quantity,
                ...
                'cis_user_sta_adhoc_position_2': f"""
                    SELECT
                        ...
                        isin_code                               AS isin,
                        NULL                                    AS ticker,
                        CAST(qty_held AS STRING)                AS quantity,
                        CAST(shares_issued AS STRING)           AS shares_outstanding,
                        NULL                                    AS shares_issued,
                        CAST(pct_holding AS STRING)             AS pct_holding,
                        NULL                                    AS market_price,
                        NULL                                    AS average_cost,
                        NULL                                    AS cost_fc,
                        NULL                                    AS market_value_fc,
                ...
                'cis_user_sta_adhoc_position_3': f"""
                    SELECT
                        ...
                        CAST(shares_outstanding AS STRING)   AS shares_outstanding,
                        CAST(shares_issued AS STRING)        AS shares_issued,
                        CAST(pct_holding AS STRING)          AS pct_holding,
                        CAST(market_price AS STRING)         AS market_price,
                        CAST(average_cost AS STRING)         AS average_cost,
                        CAST(cost_fc AS STRING)              AS cost_fc,
                        CAST(market_value_fc AS STRING)      AS market_value_fc,
                        CAST(net_book_value_fc AS STRING)    AS net_book_value_fc,
                        CAST(unrealized_pnl_fc AS STRING)    AS unrealized_pnl_fc,
                        CAST(provision_fc AS STRING)         AS provision_fc,
                        CAST(cost_lc AS STRING)              AS cost_lc,
                        CAST(market_value_lc AS STRING)      AS market_value_lc,
                        CAST(net_book_value_lc AS STRING)    AS net_book_value_lc,
                        CAST(unrealized_pnl_lc AS STRING)    AS unrealized_pnl_lc,
                        CAST(provision_lc AS STRING)         AS provision_lc,
                    ...
                    WHERE processing_date = '{processing_date}'
                      AND src_id = '{src_id}'
                """,
                # position_4: Comprehensive Position with Valuation
                # DDL cols: portfolio, security_full_name, product_type, security_type,
                ...
                #   country_of_incorporation, country_of_exchange, isin_code, ticker_code,
                #   industry, financial_non_financial_co, position_basis
                'cis_user_sta_adhoc_position_4': f"""
                    SELECT
                        ...
                        CAST(quantity AS STRING)                            AS quantity,
                        CAST(no_of_shares_issues_by_the_company AS STRING)  AS shares_outstanding,
                        CAST(no_of_shares_issues_by_the_company AS STRING)  AS shares_issued,
                        CAST(pct_holdings AS STRING)                        AS pct_holding,
                        NULL                                                AS market_price,
                        NULL                                                AS average_cost,
                        CAST(cost_fc AS STRING)                             AS cost_fc,
                        NULL                                                AS market_value_fc,
