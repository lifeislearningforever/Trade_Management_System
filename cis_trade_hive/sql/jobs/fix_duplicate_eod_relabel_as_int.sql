...
WHERE position_type = 'EOD'
  AND (portfolio, security_label, position_basis) IN (
...
ORDER BY portfolio, security_label, position_basis, position_id;
...
SELECT
    ...
    position_basis,
    COUNT(*) AS total_rows,
    SUM(CASE WHEN position_type = 'INT' THEN 1 ELSE 0 END) AS int_count,
    SUM(CASE WHEN position_type = 'EOD' THEN 1 ELSE 0 END) AS eod_count
FROM gmp_cis.cis_position
WHERE (portfolio, security_label, position_basis) IN (
    SELECT portfolio, security_label, position_basis
    FROM gmp_cis.cis_position
