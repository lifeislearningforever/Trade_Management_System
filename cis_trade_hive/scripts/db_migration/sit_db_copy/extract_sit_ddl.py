...
def write_output_files(result: Dict[str, Any], output_dir: Path, use_kerberos: bool = False):
    ...
    with open(deploy_script, 'w') as f:
        ...
        f.write('# Function to run SQL file -- returns impala-shell\'s real exit code so\n')
        f.write('# callers can tell success from failure instead of assuming success.\n')
        f.write('run_sql() {\n')
        ...
        f.write('    if run_sql "$DDL_FILE"; then\n')
        f.write('        echo "Tables created successfully."\n')
        f.write('    else\n')
        f.write('        echo "ERROR: Table creation failed (see impala-shell output above). Aborting."\n')
        f.write('        exit 1\n')
        f.write('    fi\n')
        f.write('else\n')
        ...
        f.write('        if run_sql "$DATA_FILE"; then\n')
        f.write('            echo "Data loaded successfully."\n')
        f.write('        else\n')
        f.write('            echo "ERROR: Data load failed (see impala-shell output above)."\n')
        f.write('            exit 1\n')
        f.write('        fi\n')
        f.write('    else\n')
