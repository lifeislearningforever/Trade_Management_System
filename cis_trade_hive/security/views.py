...
def security_list(request: HttpRequest) -> HttpResponse:
    ...
    if export == 'csv':
        ...
        writer.writerow([
            'Security Name', 'ISIN', 'Description', 'Ticker', 'Issuer',
            'Security Type', 'Security Sub Type', 'Investment Type', 'Security Investment',
            'Issuer Type', 'Industry', 'Quoted/Unquoted', 'Market',
            'Country of Incorporation', 'Country of Exchange', 'Country of Issue',
            'Country of Primary Exchange', 'Exchange Code', 'Currency Code',
            'Shares Outstanding', 'Beta', 'Par Value',
            'PCT HLD Entity 1', 'PCT HLD Entity 2', 'PCT HLD Entity 3', 'PCT HLD Entity AGGR',
            'Substantial 10%', 'PEVC S32 Devest', 'S32 Representative',
            'Base LIV Fund', 'MAS 643 Entity Type', 'MAS 6D Code', 'Fin/Non-fin IND',
            'Business Unit Head', 'Person In Charge', 'Core/Non-core',
            'Fund/Index Fund', 'Management Limit Classification', 'Relative Index',
            'Status', 'Source System', 'Is Active',
            'Submitted For Approval At', 'Submitted By',
            'Reviewed At', 'Reviewed By', 'Review Comments',
            'Created By', 'Created At', 'Updated By', 'Updated At',
            'UDF Country Issue', 'Price Source',
            'Fintech Speculative', 'Unlisted Eq Speculative', 'Related Company', 'Approved S32',
        ])
        ...
        for security in securities:
            writer.writerow([
                ...
                security.get('security_sub_type', ''),
                security.get('investment_type', ''),
                security.get('security_investment', ''),
                security.get('issuer_type', ''),
                security.get('industry', ''),
                security.get('quoted_unquoted', ''),
                security.get('market', ''),
                security.get('country_of_incorporation', ''),
                security.get('country_of_exchange', ''),
                security.get('country_of_issue', ''),
                security.get('country_of_primary_exchange', ''),
                security.get('exchange_code', ''),
                security.get('currency_code', ''),
                security.get('shares_outstanding', ''),
                security.get('beta', ''),
                security.get('par_value', ''),
                security.get('pct_hld_entity_1', ''),
                security.get('pct_hld_entity_2', ''),
                security.get('pct_hld_entity_3', ''),
                security.get('pct_hld_entity_aggr', ''),
                security.get('substantial_10_pct', ''),
                security.get('pevc_s32_devest', ''),
                security.get('s32_representative', ''),
                security.get('base_liv_fund', ''),
                security.get('mas_643_entity_type', ''),
                security.get('mas_6d_code', ''),
                security.get('fin_nonfin_ind', ''),
                security.get('business_unit_head', ''),
                security.get('person_in_charge', ''),
                security.get('core_noncore', ''),
                security.get('fund_index_fund', ''),
                security.get('management_limit_classification', ''),
                security.get('relative_index', ''),
                security.get('status', ''),
                security.get('src_system', ''),
                security.get('is_active', ''),
                security.get('submitted_for_approval_at', ''),
                security.get('submitted_by', ''),
                security.get('reviewed_at', ''),
                security.get('reviewed_by', ''),
                security.get('review_comments', ''),
                security.get('created_by', ''),
                security.get('created_at', ''),
                security.get('updated_by', ''),
                security.get('updated_at', ''),
                security.get('udf_country_issue', ''),
                security.get('price_source', ''),
                security.get('fintech_speculative', ''),
                security.get('unlistedeq_speculative', ''),
                security.get('related_company', ''),
                security.get('approved_s32', ''),
            ])
    ...
    if request.method == 'POST':
        try:
            ...
            security_data = {
                ...
                'security_type': request.POST.get('security_type', '').strip() or 'COMMON STOCK',
                'security_sub_type': request.POST.get('security_sub_type', '').strip(),
                'investment_type': request.POST.get('investment_type', '').strip(),
                'security_investment': request.POST.get('security_investment', '').strip(),
                ...
                'country_of_primary_exchange': request.POST.get('country_of_primary_exchange', '').strip(),
                'udf_country_issue': request.POST.get('udf_country_issue', '').strip(),
                ...
                'price_source': request.POST.get('price_source', '').strip(),
                ...
                'approved_s32': request.POST.get('approved_s32', '').strip(),
                'fintech_speculative': request.POST.get('fintech_speculative', '').strip(),
                'unlistedeq_speculative': request.POST.get('unlistedeq_speculative', '').strip(),
                'related_company': request.POST.get('related_company', '').strip(),
                ...
                'base_liv_fund': request.POST.get('base_liv_fund', '').strip(),
