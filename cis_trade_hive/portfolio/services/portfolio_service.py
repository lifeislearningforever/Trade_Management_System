...
class PortfolioService:
    ...
    def create_portfolio(user: User, data: Dict) -> Portfolio:
        ...
        AuditLog.log_action(
            ...
            description=f"Created portfolio {portfolio.code}: {portfolio.name}"
        )
    ...
    def update_portfolio(portfolio: Portfolio, user: User, data: Dict) -> Portfolio:
        ...
        if changes:
            AuditLog.log_action(
                ...
                object_type='Portfolio',
                object_id=str(portfolio.id),
                description=f"Updated portfolio {portfolio.code}: {'; '.join(changes)}"
            )
    ...
    def submit_for_approval(portfolio: Portfolio, user: User) -> Portfolio:
        ...
        AuditLog.log_action(
            ...
            object_type='Portfolio',
            object_id=str(portfolio.id),
            description=f"Submitted portfolio {portfolio.code} for approval"
        )
    ...
    def approve_portfolio(portfolio: Portfolio, user: User, comments: str = '') -> Portfolio:
        ...
        AuditLog.log_action(
            ...
            object_type='Portfolio',
            object_id=str(portfolio.id),
            description=f"Approved portfolio {portfolio.code}{comment_text}"
        )
    ...
    def reject_portfolio(portfolio: Portfolio, user: User, comments: str) -> Portfolio:
        ...
        AuditLog.log_action(
            ...
            object_type='Portfolio',
            object_id=str(portfolio.id),
            description=f"Rejected portfolio {portfolio.code} - Reason: {comments}"
        )

        return portfolio

    @staticmethod
    def close_portfolio(portfolio: Portfolio, user: User, reason: str = '') -> Portfolio:
        """
        Close an active portfolio.

        Args:
            portfolio: Portfolio to close
            user: User closing the portfolio
            reason: Closure reason

        Returns:
            Closed Portfolio instance
        ...
        if portfolio.status != 'ACTIVE':
            raise ValidationError(
                f"Only ACTIVE portfolios can be closed. Current status: {portfolio.status}"
            )
        ...
        AuditLog.log_action(
            ...
            object_type='Portfolio',
            object_id=str(portfolio.id),
            description=f"Closed portfolio {portfolio.code}" + (f" - {reason}" if reason else "")
        )
